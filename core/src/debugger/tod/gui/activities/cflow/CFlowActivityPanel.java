...
import tod.gui.components.eventlist.ConstructorChainingNode;
import tod.gui.components.eventlist.EventListPanel;
import tod.gui.components.eventlist.ExceptionGeneratedNode;
import tod.gui.components.eventlist.FieldWriteNode;
import tod.gui.components.eventlist.InstanceOfNode;
import tod.gui.components.eventlist.InstantiationNode;
import tod.gui.components.eventlist.IntimacyLevel;
import tod.gui.components.eventlist.LocalVariableWriteNode;
import tod.gui.components.eventlist.MethodCallNode;
import tod.gui.components.eventlist.NewArrayNode;
...
//	
	...
	public void init()
	{
		...
		//adding test UI elements -kai
		add(createTestPanel(), BorderLayout.BEFORE_FIRST_LINE);
	...
	//test func -kai
	private void forwardStepIntoTest() 
	{
		ILogEvent aEvent = getSeed().pSelectedEvent().get();
		
		itsStepper.forwardStepInto();
		ILogEvent nextEvent = itsStepper.getCurrentEvent();
		
		IBehaviorCallEvent theEvent = (IBehaviorCallEvent) nextEvent;
		
		String display = null;
		
		if(theEvent.getExecutedBehavior() != null)
		{
			display = "Method Name: " + theEvent.getExecutedBehavior().getName()
							+"\n\n Declaring Type: "+ theEvent.getExecutedBehavior().getDeclaringType()
							+"\n\n Timestamp: "+ theEvent.getTimestamp()
							+"\n\n Depth: "+ theEvent.getDepth()
							+"\n\n Behaviour: "+ theEvent.getExecutedBehavior().getBehaviourKind()
							+"\n\n LineNumber"+ theEvent.getExecutedBehavior().getLineNumbers()
							+"\n\n Src File: "+ theEvent.getExecutedBehavior().getSourceFile()							
							+"\n\n Local Var: "+ theEvent.getExecutedBehavior().getLocalVariables()
							+"\n\n Return Type: "+ theEvent.getExecutedBehavior().getReturnType();
		}else
		{
			display = "Method Name: " + theEvent.getCalledBehavior().getName()
					+"\n\n Declaring Type: "+ theEvent.getCalledBehavior().getDeclaringType()
					+"\n\n Timestamp: "+ theEvent.getTimestamp()
					+"\n\n Depth: "+ theEvent.getDepth()
					+"\n\n Behaviour: "+ theEvent.getCalledBehavior().getBehaviourKind()
					+"\n\n LineNumber"+ theEvent.getCalledBehavior().getLineNumbers()
					+"\n\n Src File: "+ theEvent.getCalledBehavior().getSourceFile()							
					+"\n\n Local Var: "+ theEvent.getCalledBehavior().getLocalVariables()
					+"\n\n Return Type: "+ theEvent.getCalledBehavior().getReturnType();
		}
	...
	//create panel for UI test components -kai
	private JComponent createTestPanel()
	{
		JPanel testPanel = new JPanel();
		
		//Adding Test Button -kai
		testPanel.add(new JButton(new SimpleAction(
						Resources.ICON_FORWARD_STEP_INTO.asIcon(20), 
						"Test Btn")
				{
					public void actionPerformed(ActionEvent aE)
